...
impl Config {
    ...
    pub fn load(path: &str) -> Result<Config, String> {
        ...
        if !std::path::Path::new(&(CONFIG_ROOT.to_owned() + path)).exists() {
            ...
            debug!("Config \"{}\" does not exist, creating...", path);
            let config = serde_json::json!({});
            let config_str = serde_json::to_string_pretty(&config).map_err(|e| e.to_string())?;
            std::fs::write(CONFIG_ROOT.to_owned() + path, config_str.clone())
                .map_err(|e| e.to_string())?;
            debug!("Created config at {}", path);
            debug!("Config contents: {}", config_str);
            return Ok(Config {
            ...
            debug!("Config \"{}\" exists, loading...", path);
